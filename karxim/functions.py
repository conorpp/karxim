...
def set_cookie(response, key, value, days_expire = 20, signed=True):
    ...
    if signed:
        sign = hmac.new(KEY, msg=session, digestmod=hashlib.sha1).hexdigest()
        response.set_signed_cookie(key, value+':'+sign, max_age=max_age, expires=expires)
    else: response.set_cookie(key, value, max_age=max_age, expires=expires)

def cookieValue(value):
    """returns signed value for sign cookie algorithm"""
    sign = hmac.new(KEY, msg=session, digestmod=hashlib.sha1).hexdigest()
    return value+':'+sign
